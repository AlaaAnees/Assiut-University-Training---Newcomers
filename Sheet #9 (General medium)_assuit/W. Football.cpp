...
int main() {
    IAMTRYINGandCRYING;
    int n,cnt=0,maxi=0,idx=0; cin>>n;
    string s[100];
    ...
    for(int i=0;i<n;i++){
        if(s[i]==s[i+1]) cnt++;
            ...
            if(maxi<cnt) maxi=cnt,idx=i,cnt=1;
        }
    }
    cout<<s[idx];
}
...
int main()
{
    ...
    while(--n){
        cin>>Ahly;
        score[Ahly]++;
        if(Ahly!=juventus) Barca=Ahly;
    }
    cout<<(score[juventus] > score[Barca]? juventus:Barca);
